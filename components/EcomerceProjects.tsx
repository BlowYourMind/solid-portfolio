...
import { ECOMMERCE_PROJECTS, ECOMMERCE_SKILLS } from "@/config/ecommerce";
...
const EcommerceMain = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-6 mt-40">
        ...
        <div className="mb-16">
          <div className="grid md:grid-cols-1 gap-8">
            {ECOMMERCE_PROJECTS.map((project, index) => (
              <ProjectDescriptionCard key={index} project={project} />
            ))}
          </div>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ECOMMERCE_SKILLS.map((skill, index) => (
              <SkillsList skill={skill} key={index} />
            ))}
          </div>
        </div>
